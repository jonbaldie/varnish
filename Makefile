...
test-e2e-hard: test-e2e-harness-module test-e2e-scenario-config test-hostile-static-cookie test-hostile-static-cookie-canary test-hostile-account-cookie-isolation test-hostile-set-cookie-isolation test-hostile-query-suffix test-hostile-query-suffix-canary test-5xx-not-cached test-purge-unauthorized test-post-not-cached test-hostile-post-canary test-grace-stale test-hostile-vary-star test-hostile-vary-star-canary test-hostile-authorization test-hostile-authorization-canary test-hostile-zero-ttl test-hostile-zero-ttl-canary test-hostile-invalid-expires-canary
...
test-hostile-invalid-expires-canary:
	@echo "=== Test: Hostile invalid Expires depends on the shared cache policy rule ==="
	@set -euo pipefail; \
	log_file=$$(mktemp); \
	trap 'rm -f "$$log_file"' EXIT; \
	if HOSTILE_CACHE_POLICY_PATH="$$(pwd)/test/e2e/fixtures/cache-policy-invalid-expires-cached.vcl" ./test/e2e/run-hostile-scenario.sh zero-ttl >"$$log_file" 2>&1; then \
		echo "FAIL: hostile zero-ttl scenario passed without the invalid-Expires rule"; \
		cat "$$log_file"; \
		exit 1; \
	fi; \
	if ! grep -Eq "expires-zero.css second request" "$$log_file"; then \
		echo "FAIL: invalid-Expires canary should fail on the expires-zero assertion"; \
		cat "$$log_file"; \
		exit 1; \
	fi; \
	echo "OK: hostile zero-ttl scenario failed without the invalid-Expires rule"
